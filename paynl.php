...
class plgHikashoppaymentPaynl extends hikashopPaymentPlugin
{
	...
	var $accepted_currencies = array(
          "AUD","BGN","CAD","CHF","CNY","CZK","DKK","EEK","EUR","GBP","HKD","HRK","HUF","IDR","ZAR","SGD","SKK",
          "INR","ISK","JPY","KRW","LTL","LVL","MXN","MYR","NOK","NZD","PHP","PLN","RON","RUB","SEK","THB","TRY","USD");
	...
	function onAfterOrderConfirm(&$order,&$methods,$method_id)
	{
		...
		{
			$this->app->enqueueMessage('You have to configure a token api for the Pay.nl plugin payment first : check your plugin\'s parameters, on your website backend','error');
			return false;
		}
		...
		else
		{
			if (!class_exists('Pay_Api_Start')) {
                        ...
                        foreach($order->cart->products as $product){
                          $amount =round($product->order_product_price, (int)$this->currency->currency_locale['int_frac_digits'])*100;
                          $paynlService->addProduct($product->order_product_id,$product->order_product_name,$amount,$product->order_product_quantity);
                        }
                        //shippment
                        if(!empty($order->order_shipping_price)){
                          $paynlService->addProduct('shippment',  $order->order_shipping_method,$order->order_shipping_price*100,1);
                        }
                        //tax
                        if(!empty($order->order_tax_info['VAT']->tax_amount))
                          $paynlService->addProduct('tax', $order->order_tax_info['VAT']->tax_namekey,round($order->order_tax_info['VAT']->tax_amount,(int)$this->currency->currency_locale['int_frac_digits']) * 100, 1);
                        
                        //coupon
                        if(!empty($order->order_discount_price))
                         $paynlService->addProduct('discount',$order->order_discount_code, $order->order_discount_price * -100, 1);
	...
	function onPaymentNotification(&$statuses)
	{
		...
		$history->data = ob_get_clean();
                ...
                if($order_status == 'PAID' && !$this->isPaid($order_id)){
                  ...
                  $email = new stdClass();
                  $email->subject = JText::sprintf('PAYMENT_NOTIFICATION_FOR_ORDER','Paynl',$custom_state,$dbOrder->order_number);
                  $email->body = str_replace('<br/>',"\r\n",JText::sprintf('PAYMENT_NOTIFICATION_STATUS','Paynl',$custom_state)).' '.JText::sprintf('ORDER_STATUS_CHANGED',$custom_state)."\r\n\r\n".$order_text;
                  $history->notified = 1;
                ...
                if(!$this->isPaid($dbOrder->order_id) && $order_status == 'CANCEL'){
                  ...
                  $email = new stdClass();
                  $email->subject = JText::sprintf('NOTIFICATION_REFUSED_FOR_THE_ORDER','Paynl',$custom_state,$dbOrder->order_number);
                  $email->body = str_replace('<br/>',"\r\n",JText::sprintf('NOTIFICATION_REFUSED_FOR_THE_ORDER','Paynl',$custom_state)).' '.JText::sprintf('ORDER_STATUS_CHANGED',$custom_state)."\r\n\r\n".$order_text;
